...
public class ProductDisplayDAOImpl implements ProductDisplayDAO {
    ...
    public void addProduct(ProductDisplay product) throws SQLException {
        String insertProductSQL = "INSERT INTO Products (ProductID, ProductName, ProductDescription) VALUES (?, ?, ?)";
        String insertProductDetailsSQL = "INSERT INTO Product_Details (Id, Prod_id, QuantityRemaining, Price, SKU) VALUES (?, ?, ?, ?, ?)";
        ...
        try (Connection connection = getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    String pName = resultSet.getString("Products.ProductName");
                    String pDescription = resultSet.getString("Products.ProductDescription");
                    int productId = resultSet.getInt("Product_Details.Id");
                    int quantityRemaining = resultSet.getInt("Product_Details.QuantityRemaining");
    ...
    public void updateProduct(ProductDisplay product) throws SQLException {
        String updateProductSQL = "UPDATE Products SET ProductName = ?, ProductDescription = ? WHERE ProductID = ?";
        String updateProductDetailsSQL = "UPDATE Product_Details SET QuantityRemaining = ?, Price = ?, SKU = ? WHERE Id = ?";
